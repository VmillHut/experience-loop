...
| Agent 继续负责 | 你保留并增长 |
| --- | --- |
| 分析、实现、测试、验证与交付 | 定义正确问题、理解系统边界、审查证据、做工程取舍、承担真实结果 |

它不把真实任务改造成课程，也不要求你手写本可由 Agent 完成的工作。它只在原生流程上增加三件事：

| 原则 | 意味着什么 |
| --- | --- |
| `DELIVERY FIRST` **交付优先** | 学习层只做加法：安全、正确性、验证范围和交付效率不能降级。 |
| `DECIDE FROM EVIDENCE` **先检测，再决定** | `auto` 随任务证据判断是否值得介入，以及应该安静、解释、提问还是运行短循环。 |
| `COMPOUND JUDGMENT` **判断可迁移** | 只有预测、纠正、真实结果和后续复用才算增长；聊天变长、代码生成成功都不算。 |
...
`auto` 不是「少打扰」模式，也不是「多提问」模式。它像一枚跟随任务证据变化的小雷达，持续判断：**这里有没有值得留给人的工程判断，现在是不是合适的介入时机。**
...
| 模式 | 谁决定强度 | 你会实际感受到什么 |
| --- | --- | --- |
| **`auto`** · 默认 | Agent 根据当前证据持续决定 | 可能全程安静，也可能解释、可选提问、等待一次关键判断，或运行一个短训练循环。 |
...
| --- | --- | --- |
| **日常修改** | `实现这个缓存失效需求，本周要提测。` | 验收明确时直接完成；只有边界判断值得保留时，才补充证据或安排一个最小检查点。 |
| **高价值判断** | `线上偶发重复扣款，帮我定位并修复。` | 当前证据能区分关键根因且恢复不紧急时，先让你做一次可验证预测，然后真的等待。 |
| **定向练习** | `使用 focus，我想练习测试设计。` | 围绕同一能力目标组织短而有边界的预测、审查与复盘。 |
| **架构推演** | `使用 deep 分析这次状态同步设计，先不要改代码。` | 先建模、比较方案与二阶影响、预测失效条件，再审查实现与证据。 |
| **故障与赶工** | `先恢复发布，确认健康后再复盘。` | 安全、恢复、截止期和交付优先；`off` 或「只交付」时不追加学习尾巴。 |
...
| 能力 | 一句话示例 | 默认边界 |
| --- | --- | --- |
| **调整画像** | `记住：我做后端约 4 年，希望强化可靠性判断；解释先给结论，高价值节点让我先预测。` | 只更新你提到的字段，不编造缺失信息；画像不能降低工程标准或验证范围。 |
| **临时资料** | `结合这篇文章审查当前方案：C:\Docs\article.pdf` | 默认只读与当前问题相关的部分；长期导入 Knowledge Lens 前会征得同意。 |
| **结构化数据** | `分析 C:\Data\reviews.csv，找出最常漏掉的测试类型。` | CSV、JSON、表格和日志默认只服务当前任务，不强迫建库或改配置。 |
| **优质项目** | `只读参考 D:\Repos\excellent-project 的测试架构，不要照抄。` | 参考项目与当前项目始终分离，只比较机制、约束和可验证证据。 |
