...
- **Agent 继续负责：**分析、实现、测试、验证与交付。
- **你保留并增长：**定义正确问题、理解系统边界、审查证据、做工程取舍、承担真实结果。

　　它不把真实任务改造成课程，也不要求你手写本可由 Agent 完成的工作。它只在原生流程上增加三件事：

| 原则 | 意味着什么 |
| --- | --- |
| **交付优先** | `DELIVERY FIRST` · 学习层只做加法：安全、正确性、验证范围和交付效率不能降级。 |
| **证据决策** | `DECIDE FROM EVIDENCE` · `auto` 随任务证据判断是否值得介入，以及应该安静、解释、提问还是运行短循环。 |
| **判断迁移** | `COMPOUND JUDGMENT` · 只有预测、纠正、真实结果和后续复用才算增长；聊天变长、代码生成成功都不算。 |
