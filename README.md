...
不需要改项目提示词，也不需要先学一套命令。默认的 `auto` 会根据工期、风险、不确定性和当前任务的成长价值自动调节，不要求你每次选模式。

以后想客制化也不需要打开配置表：补一句“我现在负责支付链路，希望加强可靠性”，给一个文章或数据路径，或说“把 `D:\Repos\reference-project` 作为测试架构参考”。Agent 会自动更新必要画像、建立资料索引或只读扫描参考项目，其余设置保持默认。
...
Skill 会在内部从问题定义、系统理解、验证、可靠性、Agent 编排和端到端责任中最多选择一个最值得强化的方向。它依据真实决策和验证证据调整，而不是让你维护能力表或完成课程清单。
...
默认 `auto` 会正常分析、修改和验证。低风险任务保持安静；遇到关键分岔时最多给出一次可挑战的判断；赶工或故障时先恢复；高风险任务则自动加强证据、回滚和验收覆盖。
...
```text
使用 focus 分析这次状态同步设计。列出方案、代价和失效条件，先不要开始改代码。
```

`focus` 适合主动训练、方案评审和迁移练习，不建议在所有日常任务中开启。

## 会不会拖慢开发？

默认不会明显拖慢。`auto` 要求 **0 个学习回答**，最多出现 1 个可跳过的短检查点；它会自己识别赶工、故障和高风险场景。只有想主动训练时才切到 `focus`，完全不需要学习层时切到 `off`。

| 模式 | 什么时候用 | 对工作节奏的影响 |
| --- | --- | --- |
| `auto` | 几乎所有日常工作，默认模式 | 内部动态调节；0 个必答学习问题，最多 1 个可选检查点 |
| `focus` | 主动训练、架构推演、深度复盘 | 1–2 次有目的的预测、取舍或审查 |
| `off` | 只想完成任务 | 不加入学习层，也不记录事件 |

你可以在任何一句需求里切换模式，不需要重新 setup。旧版的 `ship` / `incident` 会自动映射到 `auto`，`coach` / `deep` 会映射到 `focus`，升级无需手工迁移。
