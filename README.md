...
安装完成后，开启一个新的 Codex 会话，直接交代工作即可：

```text
用 $experience-loop 帮我定位这次登录重连问题并修复，今天要提测。
```

不需要先运行 setup 或扫描项目。想长期记住个人偏好时，再补一句“请记住：我主要负责客户端开发，偏好先给结论再解释失效条件”；只有明确需要跨任务复用项目画像时，才说“保存当前项目画像”。

不需要改项目提示词，也不需要先学一套命令。默认的 `auto` 不等于低干预：它会综合任务后果、不确定性、可迁移价值、与你画像的相关度、时间压力和交互成本，在静默、嵌入式说明、1 个检查点或最多 2 个短检查点之间选择。所有学习检查点都可跳过，`auto` 也绝不会暗自升级为 `deep`。

以后想客制化也不需要打开配置表：补一句“我现在负责支付链路，希望加强可靠性”；给一个文章或资料路径时默认只读取本次相关内容，只有复用、精确引用或跨任务检索值得时才建立 Knowledge Lens 索引；给 CSV、JSON 等结构化数据时做针对当前问题的一次性分析；或说“把 `D:\Repos\reference-project` 作为测试架构参考”，让 Agent 只读检查相关路径。其余设置保持默认。
...
Skill 会在内部从问题定义、系统理解、验证、可靠性、Agent 编排和端到端责任中最多选择一个最值得强化的方向。这个“单一学习切口”只限制学习干预和经验记录，不会缩窄代码审查、风险检查或验证覆盖。它依据真实决策和验证证据调整，而不是让你维护能力表或完成课程清单。
...
默认 `auto` 会正常分析、修改和验证，并按预期净收益选择静默、嵌入式说明或一到两个短检查点。任务后果、可迁移价值和画像相关度越高，越值得投入注意力；时间压力和交互成本越高，越倾向保持安静。机械任务、明确的 “delivery only” 和紧急恢复直接走快路径，高风险任务则加强证据、回滚和验收覆盖，而不是偷偷进入 `deep`。

### 主动训练：先预测，再看证据

```text
这次使用 focus。我想练习根因定位，在看到决定性日志前先让我判断一次。
```
...
处理期间不会插入教学问题。恢复和验证完成后，只有用户已请求复盘或当前模式仍需要它时，才补一段精简时间线；`off` 或明确的 “delivery only” 永远没有学习尾巴。

### 深度推演：开足马力，但不牺牲任务质量

```text
使用 deep 分析这次状态同步设计。完整建立心智模型，列出方案、代价、失效条件和验证证据，先不要开始改代码。
```

`deep` 只由用户显式开启，适合完整设计推演、深度调试或阶段性成长审查。它会投入更多对话和时间，但仍由任务正确性、验证、安全和有用的 Agent 执行优先；不会为了“练习”隐藏答案、强迫手写或制造困难。

## 会不会拖慢开发？

默认不会明显拖慢。`auto` 要求 **0 个必答学习回答**，会在静默、嵌入式说明、1 个检查点或最多 2 个可跳过的短检查点之间做净收益选择。想显式锁定一个能力目标时使用 `focus`，想进行全强度深度学习时使用 `deep`，完全不需要学习层时切到 `off`。

| 模式 | 什么时候用 | 对工作节奏的影响 |
| --- | --- | --- |
| `auto` | 没有当前任务显式模式或已保存默认时使用；唯一可由 Agent 自行推断的模式 | 按净收益选择静默、嵌入式说明、1 个或最多 2 个可跳过的短检查点；不会隐式升级 `deep` |
| `focus` | 显式锁定一个能力目标 | 围绕该目标安排 1–2 次有目的的预测、取舍或审查 |
| `deep` | 显式要求完整推演、深度调试或成长审查 | 全强度建立模型、比较证据、分析失效条件并做迁移练习；任务质量始终优先 |
| `off` | 只想完成任务 | 不消费内容型学习画像、不加提示或学习总结、不记录事件；仅允许最小控制面读取来识别已保存的 `off`，明确要求的资料或数据任务仍会执行 |

你可以在任何一句需求里切换模式，不需要重新 setup；这种切换默认只作用于当前任务，只有明确说“设为默认”才会持久化。`focus` 和 `deep` 不会由任务复杂度自动推断；旧版的 `ship` / `incident` 会自动映射到 `auto`，`coach` 会映射到 `focus`，`deep` 保持为一等模式，升级无需手工迁移。
...
CSV、JSON 等结构化数据默认按当前问题做一次性分析，不会为了“统一入口”强行塞进 Knowledge Lens。对于优质或示范项目，只需提供路径和一个比较问题；Agent 会保持只读、只看相关路径，并把它与当前项目分开，比较机制和证据而不是盲目照搬。
...
- [自适应工作流](references/workflow.md)：内部动态控制、委托、验收与复盘策略
