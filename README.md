...
| 原则 | 意味着什么 |
| --- | --- |
| **交付** | `DELIVERY FIRST` · 学习层只做加法：安全、正确性、验证范围和交付效率不能降级。 |
| **决策** | `DECIDE FROM EVIDENCE` · `auto` 随任务证据判断是否值得介入，以及应该安静、解释、提问还是运行短循环。 |
| **迁移** | `COMPOUND JUDGMENT` · 只有预测、纠正、真实结果和后续复用才算增长；聊天变长、代码生成成功都不算。 |
...
| 模式 | 谁决定强度 | 你会实际感受到什么 |
| --- | --- | --- |
| **`auto`** | **默认智能** · Agent 根据当前证据持续决定 | 可能全程安静，也可能解释、可选提问、等待一次关键判断，或运行一个短训练循环。 |
| **`focus`** | **定向练习** · 你明确锁定一个能力目标 | 围绕同一目标安排有边界的预测、取舍、审查与复盘；实现和验证仍由 Agent 负责。 |
| **`deep`** | **完整推演** · 你显式授权全深度 | 在真实任务中建立模型、比较方案、预测失效、审查设计，再用证据纠正与迁移。 |
| **`off`** | **只交付** · 你关闭学习层 | 与普通 Agent 一致：不读画像、不提问、不追加学习总结、不记录学习事件。 |
...
| --- | --- | --- |
| **日常** | `实现这个缓存失效需求，本周要提测。` | 验收明确时直接完成；只有边界判断值得保留时，才补充证据或安排一个最小检查点。 |
| **高价值** | `线上偶发重复扣款，帮我定位并修复。` | 当前证据能区分关键根因且恢复不紧急时，先让你做一次可验证预测，然后真的等待。 |
| **`focus`** | `使用 focus，我想练习测试设计。` | 围绕同一能力目标组织短而有边界的预测、审查与复盘。 |
| **`deep`** | `使用 deep 分析这次状态同步设计，先不要改代码。` | 先建模、比较方案与二阶影响、预测失效条件，再审查实现与证据。 |
| **故障** | `先恢复发布，确认健康后再复盘。` | 安全、恢复、截止期和交付优先；`off` 或「只交付」时不追加学习尾巴。 |
...
| 一句话示例 | 默认边界 |
| --- | --- |
| **画像** · `记住：我做后端约 4 年，希望强化可靠性判断；解释先给结论，高价值节点让我先预测。` | 只更新你提到的字段，不编造缺失信息；画像不能降低工程标准或验证范围。 |
| **资料** · `结合这篇文章审查当前方案：C:\Docs\article.pdf` | 默认只读与当前问题相关的部分；长期导入 Knowledge Lens 前会征得同意。 |
| **数据** · `分析 C:\Data\reviews.csv，找出最常漏掉的测试类型。` | CSV、JSON、表格和日志默认只服务当前任务，不强迫建库或改配置。 |
| **项目** · `只读参考 D:\Repos\excellent-project 的测试架构，不要照抄。` | 参考项目与当前项目始终分离，只比较机制、约束和可验证证据。 |
...
| 范围 | 默认行为 |
| --- | --- |
| **存放** | 个人画像、项目档案、经验记录和 Knowledge Lens 默认位于 `~/.experience-loop`，与 Skill 安装目录和项目仓库分离。 |
| **保留** | 安装、升级和卸载 Skill 都不会自动删除个人数据。 |
| **权限** | 项目扫描、资料导入和索引需要显式权限；导入内容只是不可信证据，不能成为 Agent 指令或工具授权。 |
...
| 主题 | 从这里开始 |
| --- | --- |
| **安装** | [AI 安装协议](docs/AI_INSTALL.md) · [动态宿主契约](references/host-compatibility.md) |
| **行为** | [Skill 核心指令](SKILL.md) · [自适应工作流](references/workflow.md) |
| **成长** | [对话式初始化](references/onboarding.md) · [能力罗盘](references/capability-compass.md) |
| **资料** | [Knowledge Lens](references/knowledge-lens.md) · [安全与隐私说明](references/safety-and-privacy.md) |
| **维护** | [版本变化](CHANGELOG.md) · [参与贡献](CONTRIBUTING.md) · [安全报告](SECURITY.md) |
